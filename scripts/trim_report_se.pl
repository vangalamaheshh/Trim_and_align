    ...
    foreach my $trim_file( @$file_list ) {
        ...
        while( my $line = <FH> ) {
            ...
            if( $line =~ /^Input Read Pairs:/ ) {
                my( $total_reads, $surviving, $dropped ) = ( $line =~ /.+?\s(\d+)\s.+?\s(\d+)\s.+?\s(\d+)\s.+?\s/ );
                $$trim_info{ $sample } = [ $total_reads, $surviving, $dropped ];
            }
        }
...
sub print_trim_info {
    my( $trim_info ) = @_;
    print STDOUT join( "\t", qw(SampleName TotalReads Surviving Dropped) ), "\n";
